...
```
最后一条消息以裸 tool_result 收尾   ->  注入
```

后端那条要求是两段式的：**触发**看最后一块是不是裸 `tool_result`（不是就根本不校验，历史
多脏都无所谓）；**一旦触发就扫全历史**，任何一个查不到缓存的 id 都 400。所以闸门在尾块上，
末尾追加一个非空 text 块，要求根本不启动。

#### 为什么不按 tool id 前缀收窄（试过，已否决）

一度实现成"裸 `tool_result` 收尾 **AND** 历史里有非 `call_` 前缀的 id"，动机是省掉 72.9%
白打的注入（生产两天实测：9380 次触发里只有 2542 次历史带外来 id）。**这个方案是错的**，
因为那个判断不可能做对：

前缀最多能证明"这个 id 由 deepseek 经 opencode 签发"，**证明不了"由同一个 opencode 账号
签发"**。reasoning 缓存可能按账号隔离，而 newapi 有约 10 个 opencode 渠道各一把 key，
litellm 不知道这次请求被投给了哪一个，id 里也不编码账号。所以"同前缀"不等于"查得到缓存"。

两个方向的代价完全不对称：

| 判错方向 | 后果 |
|---|---|
| 漏注入 | 400 -> **该会话此后每轮都打不进 deepseek**，只能 fallback 到 qwen（单向棘轮，不可接受） |
| 多注入 | 模型多看见一句 `Continue.`，有噪音、可恢复 |

而且**那个跨账号漏洞从未被观测到过** —— 原来就是无条件注入、闸门没机会触发，所以干净会话
到底会不会 400，数据说不了。开启收窄等于拿会话去赌一个未验证的假设。

要收窄的前提是先证明 reasoning 缓存是**全局**而非按账号隔离的（验法见"待验"）。在那之前
一律注入。

#### 那么白打的注入怎么办

**靠注入物本身的质量化解，不靠减少次数。** 原来的问题不是"注多了"，是"注了个模型读不懂
的东西"：
...
换成 `Continue.` 之后，注入物在它出现的那个位置（工具刚返回、模型要决定下一步）是**真话**，
模型不需要猜它是什么意思。次数不变，噪音消失。

#### 也不要把"思考是否开启"加进条件

看着能更省 —— 400 只在思考开启时发生（实测：`thinking: disabled` 下假 id 一律 200）。但思考
的最终状态由 `thinking_switch` 归一化后再叠加 newapi 那条规则决定，本 guardrail 侧看到的
不是最终值，把两个 guardrail 的逻辑耦起来很脆，而判错的方向仍然是 400。关态多注入无害。

同理，**不要因为"思考现在默认关着"就整个删掉注入**：thinking 开关已修好并上生产
（`thinking_switch.py`，commit `b1d45ff442`），思考能真开，400 就能真回来。

#### 请求侧的 id 家族（留档）

前缀判断已删掉，这张表只作为"生产上到底有多少种签发方"的事实留存。若将来验明缓存是全局的、
重新考虑收窄，注意 `call-<uuid>` 与自己人的 `call_` 只差一个字符，判断必须精确到下划线。

| 家族 | 出现次数 | 不同 id 数 | 签发方 |
|---|---|---|---|
| `call_` | 2005338 | 18457 | deepseek 经 opencode |
| `toolu_` | 481110 | 3359 | qwen3.7-plus |
| `tc_` | 20298 | 373 | ? |
| `call-<uuid>`（**连字符**） | 10788 | 54 | ? |
| `toolu_bdrk_` | 5322 | 511 | kiro 的 claude |
| `chatcmpl-tool-` | 2482 | 15 | ? |
| `<工具名>_xxx`（`Read_g33kfzc4vw7`） | 1532 | 8 | ? |
...
一步。注入是无条件的，所以这个代价**每次触发都付**（生产两天 9380 次），不像原来那样能靠
条件收窄摊薄 —— 上线后要专门观察这一点。`.` 没这个风险，但它换来的是模型反问用户，
那个已经在生产上被证明更糟。
...
单元测试 48 项（`test_tool_result_trailing_text.py`）。变异测试 7/7 全杀：

| 变异 | 被杀测试数 |
|---|---|
| 尾块条件删掉（连已有 text 收尾也注入） | 10 |
| 用 `messages[-1]` 代替有效末条 | 6 |
| 不跳过空白 text 块 | 6 |
| 尾部 system 不跳过 | 5 |
| 注入内容改单空格 | 4 |
| 注入内容退回 `.` | 2 |
...
注入物的四条契约（非空白、不是孤立句点、纯 ASCII、就是 `Continue.`）各自独立断言 ——
注入是无条件的，所以注入物的质量是唯一防线，那里退化没有第二道闸门挡着。

### 待验

- **reasoning 缓存是全局还是按 opencode 账号隔离**。这决定能不能按 tool id 前缀收窄注入
  （见上面"为什么不按前缀收窄"）。验法：同一份带 `call_` 前缀真 id 的 payload，分别强制走
  两个不同 opencode 渠道（newapi 侧指定 channel），裸 `tool_result` 收尾、不注入、思考开着；
  若换渠道后 400 就是按账号隔离，收窄方案永久作废；若都 200 才可以谈收窄
...
但与 `tool_result_trailing_text` 有个连带效应：那个注入是为了绕开 tool id 查不到 reasoning
缓存导致的 400，而那个 400 的真因是**思考模式要求回传 `reasoning_content`**。所以思考关着时
注入是白打、开着时必需。

**不要因此删掉注入**（本 guardrail 让思考能真开，400 就能真回来），也**不要把思考状态加进
注入条件** —— 最终状态由本 guardrail 归一化后再叠加 newapi 那条规则决定，那边看到的不是
最终值，耦起来很脆，而判错的方向是 400。那个注入现在不判任何来源，无条件执行。
